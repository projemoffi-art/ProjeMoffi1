...
export default function LegendaryLightDashboard() {
    ...
    useEffect(() => {
        ...
        if (searchParams.get('open') === 'passport') {
            handleNfcScan();
        }
    }, [searchParams]);
    ...
    // Pre-load pet daily stats from Supabase on active pet change
    useEffect(() => {
        if (!activePetObj?.id) return;

        const petId = activePetObj.id;
        ...
        if (isUuid) {
            apiService.getPetDailyStats(petId, todayStr).then((dbStats) => {
                ...
                console.error("Error fetching pet daily stats on active pet change:", err);
            });
        }
    }, [activePetObj?.id, todayStr]);
                        ...
                        <div className="flex-1">
                            <div className="flex items-center justify-between">
                                ...
                                <button 
                                    ...
                                    className="p-1.5 text-gray-400 hover:text-cyan-500 rounded-full hover:bg-gray-50 transition-all cursor-pointer"
                                    title="Pasaportu Aç"
                                >
            ...
            <AnimatePresence>
                {expandedPanel && (
                    <>
                        ...
                        <div className={`fixed inset-0 z-[101] flex items-center justify-center pointer-events-none ${expandedPanel === 'profile' || expandedPanel === 'passport' ? 'p-0' : 'p-4'}`}>
                            <motion.div
                                ...
                                className={
                                    expandedPanel === 'profile' 
                                        ...
                                        : expandedPanel === 'passport'
                                            ? "w-full h-full bg-[#0B0B0E] rounded-none shadow-[0_25px_60px_rgba(0,0,0,0.5)] border-0 overflow-hidden pointer-events-auto flex flex-col overflow-y-auto no-scrollbar"
                            ...
                            >
                                ...
                                {expandedPanel === 'profile' ? (
                                    ...
                                    <div className={cn(
                                        "flex justify-between items-center px-6 pt-5 pb-3 shrink-0 border-b",
                                        expandedPanel === 'passport' 
                                            ? "bg-[#0B0B0E]/80 border-zinc-805/80 text-white" 
                                            : "bg-gray-50/50 border-gray-100/50 text-gray-500"
                                    )}>
                                        <div className="flex items-center gap-1">
                                            <div className={cn("w-2 h-2 rounded-full animate-pulse", expandedPanel === 'passport' ? "bg-cyan-500" : "bg-green-500")} />
                                            <span className={cn("text-[9px] font-black tracking-widest uppercase", expandedPanel === 'passport' ? "text-cyan-400" : "text-gray-400")}>
                                                {expandedPanel === 'passport' ? "Moffi Passport Secure Core" : "Moffi Smartwatch OS"}
                                            </span>
                                        </div>
                                        <motion.button 
                                            whileTap={{ scale: 0.9 }}
                                            ...
                                            className={cn(
                                                ...
                                                expandedPanel === 'passport' 
                                                    ...
                                                    : "bg-gray-200/60 hover:bg-gray-200 text-gray-600"
                                            )}
                                        >
                                            <X className="w-4 h-4" />
                                        </motion.button>
                                ...
                                >
                                    ...
                                    {expandedPanel === 'passport' && (
                                        <div className="text-left w-full h-full flex flex-col bg-[#0B0B0E]">
                                            <PassportTab pet={activePetObj} onClose={() => setExpandedPanel(null)} isPublic={false} />
                                        </div>
                                    )}
