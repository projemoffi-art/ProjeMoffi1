...
export default function FoodCatchGame({ onGameOver, onClose }: GameProps) {
    // 1. FİZİK VE OYUN MANTIĞI İÇİN REFLER (React State kullanmıyoruz ki takılma olmasın)
    ...
    // 2. SADECE UI BİLEŞENLERİ İÇİN REACT STATE (Skor, Zaman, Eklenen/Silinen İtemler)
    const [uiScore, setUiScore] = useState(0);
    const [uiCombo, setUiCombo] = useState(0);
    const [uiStage, setUiStage] = useState(STAGES[0]);
    const [uiTime, setUiTime] = useState(0);
    const [renderItems, setRenderItems] = useState<GameItem[]>([]); // Sadece doğma ve yok olma anında güncellenir
    ...
    const spawnItem = (currentTime: number, currentStage: typeof STAGES[0]) => {
        if (currentTime - lastSpawnTimeRef.current > currentStage.spawnRate) {
            ...
            // Sadece item eklendiğinde React'i tetikle
    ...
    const gameLoop = useCallback((timestamp: number) => {
        if (gameOverScreen) return; // Oyun bittiyse dur
        ...
        for (const item of itemsRef.current) {
            // Y pozisyonunu güncelle (Donanım hızlandırmalı pürüzsüz akış)
            item.y += (currentStage.baseSpeed * item.speedMod * deltaTime) / 16; 
            
            let caught = false;
            
            // Çarpışma Kontrolü
            if (item.y > hitZoneY - 8 && item.y < hitZoneY + 8) {
                if (Math.abs(item.x - playerXHit) < 12) { 
                    caught = true;
                    itemsChanged = true;
                    if (item.type === 'bomb') {
                        triggerShake();
            ...
            if (!caught && item.y < 110) {
                ...
                // DOĞRUDAN DOM MANİPÜLASYONU (Sıfır takılma, 60/120 FPS)
        ...
        // Karakterin DOM pozisyonunu doğrudan güncelle
        if (playerDOMRef.current) {
            playerDOMRef.current.style.transform = \`translate3d(calc(\${playerXRef.current}vw - 50%), 0, 0)\`;
        }

        // Sadece sildiğimiz bir item varsa React'i re-render et (Performans hilesi)
        if (itemsChanged) {
        ...
        // Oyun bitiş kontrolü (Örn: Boss Modunda 45 saniye hayatta kaldıysa biter)
    ...
    return (
        <div ref={containerRef} className={cn("fixed inset-0 z-[100] bg-[#050505] overflow-hidden font-sans select-none touch-none", screenShake && "animate-[shake_0.2s_ease-in-out]")}>
            ...
            {/* ITEMS (DOM Manüpilasyonu ile Pürüzsüz Animasyon) */}
            {!gameOverScreen && renderItems.map(item => (
                <div
                    ...
                    style={{ transform: \`translate3d(calc(\${item.x}vw - 50%), calc(\${item.y}vh - 50%), 0)\` }}
                >
            ...
            {/* PLAYER (Robot Moffi) - Arka plansız ve DOM Ref ile pürüzsüz hareket */}
            {!gameOverScreen && (
                <div
                    ref={playerDOMRef}
                    className="absolute bottom-[15%] left-0 w-32 h-32 pointer-events-none will-change-transform z-20"
                    style={{ transform: \`translate3d(calc(\${playerXRef.current}vw - 50%), 0, 0)\` }}
                >
                    <div className="relative w-full h-full flex flex-col items-center justify-end">
                        {/* Glowing Platform */}
                        <div className="absolute bottom-[-10px] w-32 h-6 bg-cyan-500/30 rounded-full blur-md" />
                        <div className="absolute bottom-[-5px] w-20 h-2 bg-cyan-400/70 rounded-full shadow-[0_0_20px_rgba(34,211,238,1)]" />
                        
                        {/* Character Avatar - Tamamen arkaplansız ve özgür */}
...
console.log('Successfully updated FoodCatchGame.tsx');
