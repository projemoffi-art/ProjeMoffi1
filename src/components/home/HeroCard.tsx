            ...
            <div className="relative w-full h-[30rem] rounded-[2.5rem] overflow-hidden shadow-2xl shadow-indigo-500/20 group cursor-pointer">
                ...
                <div className="absolute top-6 left-6 right-6 flex justify-between items-start z-20">
                    ...
                    <motion.button
                        ...
                        className={cn("bg-black/20 dark:bg-white/20 backdrop-blur-md border border-black/20 dark:border-white/20 rounded-3xl p-2 pr-4 flex items-center gap-3 shadow-lg transition-all active:scale-95", showWeather && "bg-white/90 border-white text-black")}
                    >
                        ...
                        <div className="text-left">
                            ...
                            <div className={cn("text-[10px] font-medium leading-none", showWeather ? "text-gray-500" : "text-black/80 dark:text-white/80")}>{WEATHER_DATA.condition}</div>
                        </div>
                    </motion.button>
                    ...
                    {/* STATUS EDIT */}
                    <div className="bg-black/30 backdrop-blur-md border border-black/10 dark:border-white/10 rounded-full px-3 py-1.5 flex items-center gap-2 text-black/90 dark:text-white/90 shadow-lg">
                            ...
                            <div className="flex justify-between gap-2">
                                {WEATHER_DATA.hourly.map((h, i) => (
                                    <div key={i} className="flex flex-col items-center gap-1 bg-card p-2 rounded-xl shadow-moffi-card w-full">
                                        <span className="text-[10px] font-bold text-gray-500 dark:text-gray-400">{h.time}</span>
                ...
                <div className="absolute bottom-0 left-0 right-0 p-8 pb-8 flex flex-col items-start z-20">
                    ...
                    <div className="mb-4">
                        ...
                        <p className="text-black/80 dark:text-white/80 text-sm font-medium flex items-center gap-2">
                    ...
                    <div className="w-full grid grid-cols-3 gap-3 mb-5">
                        {/* Walk */}
                        <button onClick={() => router.push('/walk')} className="bg-black/10 dark:bg-white/10 backdrop-blur-md rounded-2xl p-3 border border-white/15 flex flex-col items-center gap-2 hover:bg-black/20 dark:bg-white/20 transition-colors active:scale-95">
                        ...
                        {/* Food */}
                        <button onClick={handleFeed} className="bg-black/10 dark:bg-white/10 backdrop-blur-md rounded-2xl p-3 border border-white/15 flex flex-col items-center gap-2 hover:bg-black/20 dark:bg-white/20 transition-colors active:scale-95 relative overflow-hidden">
                        ...
                        {/* Love */}
                        <button onClick={handleLove} className="bg-black/10 dark:bg-white/10 backdrop-blur-md rounded-2xl p-3 border border-white/15 flex flex-col items-center gap-2 hover:bg-black/20 dark:bg-white/20 transition-colors active:scale-95">
                            <div className="relative w-12 h-12 flex items-center justify-center">
                                <svg className="w-full h-full -rotate-90" viewBox="0 0 36 36">
                                    <circle cx="18" cy="18" r="15.9155" stroke="rgba(255,255,255,0.1)" strokeWidth="3" fill="none" />
                    ...
                    <button
                        ...
                        className="w-full bg-[#5B4D9D] text-white py-4 rounded-2xl font-black text-sm flex items-center justify-center gap-2 shadow-xl hover:bg-[#4a3e80] transition-colors active:scale-95 border border-black/20 dark:border-white/20"
                    >
