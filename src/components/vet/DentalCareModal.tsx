...
const QUESTIONS = [
    {
        ...
        options: [
            { label: "Normal / Kokusuz", score: 0, icon: "🌸" },
            { label: "Hafif kokuyor", score: 1, icon: "🍃" },
            { label: "Çok kötü / Kokmuş balık gibi", score: 3, icon: "🐟" }
        ]
    },
    {
        id: 2,
...
export function DentalCareModal({ isOpen, onClose }: DentalCareModalProps) {
    ...
    return (
        <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[150] bg-black/40 backdrop-blur-md dark:bg-black/90 dark:backdrop-blur-2xl flex items-end sm:items-center justify-center p-0 sm:p-4"
        >
            <motion.div
                initial={{ y: "100%" }}
                animate={{ y: 0 }}
                exit={{ y: "100%" }}
                className="w-full max-w-lg bg-white dark:bg-[#111111] rounded-t-[3.5rem] sm:rounded-[4rem] h-[90vh] flex flex-col overflow-hidden shadow-2xl dark:shadow-[0_50px_100px_rgba(0,0,0,0.8)] border border-zinc-200 dark:border-card-border relative"
            >
                {/* iOS Style Grab Handle */}
                <div className="absolute top-3 left-1/2 -translate-x-1/2 w-12 h-1.5 bg-zinc-250 dark:bg-white/10 rounded-full sm:hidden z-50" />
                ...
                <div className="p-8 pb-4 bg-white/80 dark:bg-[#111111]/80 backdrop-blur-3xl z-30 sticky top-0 border-b border-zinc-200 dark:border-card-border">
                    <div className="flex justify-between items-center mb-8 mt-2 sm:mt-0">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h2 className="text-2xl font-black text-zinc-800 dark:text-white tracking-tighter uppercase italic leading-none">Ağız Sağlığı</h2>
                    ...
                    <div className="flex bg-zinc-100 dark:bg-white/5 border border-zinc-200 dark:border-card-border p-1.5 rounded-[1.8rem]">
                        <button
                ...
                {/* CONTENT AREA */}
                <div className="flex-1 overflow-y-auto p-8 custom-scrollbar bg-zinc-50/50 dark:bg-black/20">
                        ...
                        <div className="h-full flex flex-col">
                            {!showResult ? (
                                <AnimatePresence mode="wait">
                                    ...
                                    >
                                        <div className="text-center mb-10">
                                            <div className="inline-block bg-zinc-100 text-zinc-500 dark:bg-white/5 dark:text-white/40 px-5 py-2 rounded-full text-[10px] font-black uppercase tracking-[0.3em] mb-6 border border-zinc-200 dark:border-card-border">
                                                SORU {currentStep + 1} / {QUESTIONS.length}
                                            </div>
                                            <h3 className="text-3xl font-black text-zinc-800 dark:text-white leading-tight tracking-tighter uppercase italic">
                                ...
                                <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center pt-8">
                                    ...
                                    <h3 className="text-3xl font-black text-zinc-850 dark:text-white mb-3 tracking-tighter uppercase italic">{getResultFeedback().title}</h3>
                                    <p className="text-zinc-500 dark:text-white/40 font-bold mb-10 max-w-[90%] mx-auto leading-relaxed uppercase tracking-tight text-sm">{getResultFeedback().desc}</p>

                                    <div className="flex flex-col gap-4">
                                        {totalScore > 3 && (
                                            <button className="w-full h-16 rounded-[2rem] bg-[#FF3B30] text-white font-black uppercase tracking-[0.2em] shadow-lg shadow-red-500/25 hover:bg-red-650 active:scale-95 transition-all text-xs cursor-pointer">
                                                VETERİNER RANDEVUSU AL
                                            </button>
                                        )}
                                        <button onClick={resetCheckup} className="w-full h-16 rounded-[2rem] bg-zinc-100 dark:bg-white/5 border border-zinc-200 dark:border-card-border text-zinc-800 dark:text-white font-black uppercase tracking-[0.2em] shadow-sm active:scale-95 transition-all text-xs cursor-pointer">
                                            TEKRAR KONTROL ET
                                        </button>
                                    </div>
                                </motion.div>
                        ...
                        <div className="flex flex-col items-center justify-center min-h-[50vh] pt-12 text-center">
                            <div className="relative mb-12">
                                ...
                                <svg className="w-72 h-72 transform -rotate-90 filter drop-shadow-[0_0_20px_rgba(16,185,129,0.1)]">
                                    <circle cx="144" cy="144" r="130" stroke="currentColor" strokeWidth="6" fill="transparent" className="text-zinc-100 dark:text-white/5" />
                                    <circle
                                        cx="144" cy="144" r="130"
                    ...
                    {activeTab === 'guide' && (
                        <div className="space-y-6">
                            ...
                            <div className="bg-[#FF3B30]/5 border border-red-500/20 p-6 rounded-[2.5rem] flex gap-5 items-center">
                                ...
                                <div className="flex-1 text-left">
                                    ...
                                    <p className="text-zinc-500 dark:text-white/40 text-[10px] font-bold uppercase tracking-tight leading-relaxed text-left">Florür köpekler için zehirlidir. Sadece evcil hayvan macunu kullan.</p>
                                </div>
                            </div>

                            <div className="bg-blue-500/5 border border-blue-500/20 p-6 rounded-[2.5rem] flex gap-5 items-center">
                                <div className="w-14 h-14 bg-blue-500/10 rounded-2xl flex items-center justify-center text-blue-500 dark:text-blue-400 shrink-0 border border-blue-500/10">
                                    <Shield className="w-7 h-7" />
                                </div>
                                <div className="flex-1 text-left">
                                    <span className="font-black text-blue-500 dark:text-blue-400 text-[10px] uppercase tracking-widest block mb-1 italic">MOFFİ ÖNERİYOR</span>
                                    <p className="text-zinc-500 dark:text-white/40 text-[10px] font-bold uppercase tracking-tight leading-relaxed text-left">Enzimli diş macunları mekanik fırçalama olmadan da temizlik sağlar.</p>
                                </div>
                            </div>
                        </div>
                    )}
