...
const SightingMapSelector = dynamic(() => import('@/components/community/SightingMapSelector'), {
    ssr: false,
    loading: () => (
        <div className="w-full h-full bg-card dark:bg-[#1A1A1A] animate-pulse rounded-2xl flex items-center justify-center text-black/30 dark:text-white/20 font-bold text-[10px] uppercase tracking-wider">Harita Yükleniyor...</div>
    )
});
    ...
    const TimeWheel = ({ value, onChange, max, label }: { value: number, onChange: (v: number) => void, max: number, label: string }) => {
        ...
        return (
            <div className="flex flex-col items-center">
                <span className="text-[9px] font-black text-black/30 dark:text-white/20 mb-2 uppercase tracking-widest">{label}</span>
                    ...
                    {numbers.map(n => (
                        <div 
                            ...
                            className={cn(
                                ...
                                value === n ? "text-cyan-400 font-black text-xl scale-110" : "text-black/30 dark:text-white/20 text-sm font-bold opacity-40 hover:opacity-100"
                            )}
                        >
            ...
            {isOpen && (
                ...
                >
                    <motion.div 
                        ...
                        className="w-full h-[90vh] bg-background dark:bg-[#0A0A0E] rounded-t-[3.5rem] border-t border-card-border flex flex-col overflow-hidden shadow-[0_-20px_60px_rgba(0,0,0,0.8)]"
                        onClick={(e) => e.stopPropagation()}
                    >
                        {/* Header */}
                        <div className="px-8 py-6 flex items-center justify-between border-b border-card-border bg-background dark:bg-[#0A0A0E]/80 backdrop-blur-md sticky top-0 z-50">
                            <button onClick={handleBack} className="flex items-center gap-1 text-black/50 dark:text-white/40 font-bold text-xs uppercase tracking-widest hover:text-white transition-all active:scale-95 group">
                        ...
                        {allPets.length > 1 && (
                            <div className="px-8 py-4 bg-black/5 dark:bg-white/5 border-b border-card-border overflow-x-auto no-scrollbar flex items-center gap-4">
                                <span className="text-[9px] font-black text-black/30 dark:text-white/20 uppercase tracking-[0.2em] shrink-0">Hızlı Seçim:</span>
                                {allPets.map(p => (
                                    <button 
                                        key={p.id}
                                        ...
                                        className={cn(
                                            ...
                                            p.id === pet.id 
                                                ...
                                                : "bg-black/5 dark:bg-white/5 border-card-border text-black/50 dark:text-white/40 hover:bg-black/10 dark:bg-white/10"
                                        )}
                                    >
                            ...
                            {[
                                ...
                                <button
                                    ...
                                    className={cn(
                                        ...
                                        activeTab === tab.id 
                                            ? "bg-black/10 dark:bg-white/10 border-card-border text-white" 
                                            : "bg-transparent border-transparent text-black/40 dark:text-white/30 hover:text-black/60 dark:text-white/60"
                                    )}
                                >
                                ...
                                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
                                    <div className="flex flex-col items-center text-center">
                                        <div className={cn(
                                            ...
                                            <div className="absolute -bottom-2 -right-2 w-8 h-8 rounded-full bg-white dark:bg-black border-2 border-card-border flex items-center justify-center">
                                        ...
                                        <p className="text-black/50 dark:text-white/40 text-xs font-bold uppercase tracking-widest mt-1">Pet-ID Durum Kontrolü</p>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-1 gap-4">
                                        ...
                                        >
                                            <div className={cn(
                                                ...
                                                isLost ? "bg-card text-red-500" : "bg-black/10 dark:bg-white/10 text-black/50 dark:text-white/40"
                                            )}>
                                                <AlertTriangle className={cn("w-8 h-8", isLost ? "animate-pulse" : "")} />
                                            </div>
                                            <div>
                                                <h4 className={cn("text-xl font-black italic uppercase", isLost ? "text-white" : "text-black/50 dark:text-white/40")}>
                                                    {isLost ? "Kayıp Alarmı Aktif!" : "Kayıp Alarmı Kapalı"}
                                                </h4>
                                                <p className={cn("text-[10px] font-bold uppercase tracking-widest mt-1", isLost ? "text-red-100" : "text-black/30 dark:text-white/20")}>
                                        ...
                                        <div className="bg-black/5 dark:bg-white/5 rounded-3xl p-5 border border-card-border flex items-start gap-4">
                                            <Info className="w-5 h-5 text-cyan-400 shrink-0 mt-0.5" />
                                            <p className="text-[11px] text-black/50 dark:text-white/50 leading-relaxed font-bold">
                            ...
                            {activeTab === 'config' && (
                                <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-8">
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <div className="bg-black/5 dark:bg-white/5 border-y border-card-border divide-y divide-white/5">
                                            {/* Reward Row */}
                                            <div className="p-6 space-y-4 bg-orange-500/5">
                                                <div className="flex items-center justify-between">
                                                    <div className="flex items-center gap-2">
                                                        <label className="text-[10px] font-black text-black/50 dark:text-white/40 uppercase tracking-widest block">Kayıp Ödülü</label>
                                                        <button 
                                                            onClick={() => setLocalSos((prev: any) => ({...prev, sosConfig: {...prev.sosConfig, rewardEnabled: !prev.sosConfig.rewardEnabled}}))}
                                                            className={cn(
                                                                "w-8 h-4 rounded-full relative transition-all duration-300",
                                                                localSos.sosConfig.rewardEnabled ? "bg-orange-500 shadow-[0_0_10px_rgba(249,115,22,0.3)]" : "bg-black/10 dark:bg-white/10"
                                                            )}
                                                        >
                                                            <div className={cn(
                                                    ...
                                                    <div className="flex items-center gap-2 text-[10px] bg-black/5 dark:bg-white/5 px-2 py-1 rounded-lg border border-card-border font-bold text-orange-400">
                                            ...
                                            <div className="p-6 space-y-3 bg-cyan-500/5">
                                                ...
                                                <div className="relative">
                                                    ...
                                                    <input 
                                                        ...
                                                        className="w-full bg-cyan-400/10 border border-cyan-400/20 rounded-2xl py-4 pl-12 pr-4 text-sm font-black text-white outline-none focus:border-cyan-400/50 placeholder:text-black/30 dark:text-white/20"
                                            ...
                                            <div className="p-6 space-y-3 border-b border-black/5 dark:border-white/5 bg-cyan-500/5">
                                            ...
                                            {/* SMS Number Row */}
                                            <div className="p-6 space-y-3">
                                                <label className="text-[9px] font-black text-black/30 dark:text-white/20 uppercase tracking-widest block ml-1">Acil Durum SMS Kişisi</label>
                                                <div className="relative">
                                                    <Phone className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-black/30 dark:text-white/20" />
                                                    <input 
                                                        type="text"
                                                        ...
                                                        className="w-full bg-black/5 dark:bg-white/5 border border-card-border rounded-2xl py-4 pl-12 pr-4 text-sm font-bold text-white outline-none focus:border-cyan-500/50"
                                                        placeholder="+90 5XX XXX XX XX"
                                                    />
                                                </div>
                                                <p className="text-[8px] text-black/30 dark:text-white/20 ml-1 italic font-medium">SOS durumunda bu numaraya otomatik konum SMS'i gönderilir.</p>
                                            </div>

                                            {/* Finder Message Row */}
                                            <div className="p-6 space-y-3">
                                                <label className="text-[9px] font-black text-black/30 dark:text-white/20 uppercase tracking-widest block ml-1">Bulan Kişiye Özel Mesaj</label>
                                                <textarea 
                                                    ...
                                                    className="w-full bg-black/5 dark:bg-white/5 border border-card-border rounded-2xl p-4 text-sm text-black/80 dark:text-white/80 font-medium outline-none focus:border-cyan-500/50 h-24 resize-none"
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <div className="bg-black/5 dark:bg-white/5 border-y border-card-border divide-y divide-white/5">
                                            ...
                                            <div className="p-6 space-y-4 bg-cyan-500/5">
                                                <label className="text-[10px] font-black text-black/50 dark:text-white/40 uppercase tracking-widest block leading-none">Radar Bildirim Menzili</label>
                                                        ...
                                                        <button 
                                                            ...
                                                            className={cn(
                                                                ...
                                                                localSos.sosConfig.sosRadius === r 
                                                                    ...
                                                                    : "bg-black/5 dark:bg-white/5 border-card-border text-black/40 dark:text-white/30 hover:text-black/60 dark:text-white/60"
                                                            )}
                                                        >
                                                            {r === 'city' ? 'Şehir' : r}
                                                        </button>
                                                ...
                                                <p className="text-[8px] text-black/30 dark:text-white/20 italic font-medium">Başkalarının ilanlarını ne kadar mesafeden duymak istersiniz?</p>
                                            </div>

                                            {/* Location Precision Row */}
                                            <div className="p-6 space-y-4">
                                                <label className="text-[10px] font-black text-black/50 dark:text-white/40 uppercase tracking-widest block leading-none">Konum Hassasiyeti</label>
                                                    ...
                                                    {(['exact', 'area'] as const).map((p) => (
                                                        ...
                                                        >
                                                            <div className="relative z-10">
                                                                <p className={cn("text-xs font-black uppercase tracking-wider mb-1", localSos.sosConfig.locationPrecision === p ? "text-emerald-400" : "text-black/50 dark:text-white/40")}>
                                                                    {p === 'exact' ? 'Nokta Atışı' : 'Genel Bölge'}
                                                                </p>
                                                                <p className="text-[9px] text-black/30 dark:text-white/20 font-bold leading-tight">
                                                ...
                                                {[
                                                    ...
                                                    <div key={item.id} className="flex items-center justify-between p-6">
                                                        <div className="flex items-center gap-4">
                                                            ...
                                                            <span className="text-[11px] font-bold text-black/80 dark:text-white/80">{item.label}</span>
                                                        </div>
                                                        <button 
                                                            ...
                                                            className={cn(
                                                                ...
                                                                (localSos.sosConfig as any)[item.id] ? "bg-cyan-500 shadow-[0_0_15px_rgba(6,182,212,0.3)]" : "bg-black/10 dark:bg-white/10"
                                                            )}
                                                        >
                                                            <div className={cn(
                                            ...
                                            <div className="p-6 space-y-6">
                                                <div className="flex items-center justify-between">
                                                    <div className="flex items-center gap-4">
                                                        <div className="w-10 h-10 rounded-xl bg-black/5 dark:bg-white/5 flex items-center justify-center">
                                                            <Clock className={cn("w-5 h-5", localSos.sosConfig.quietHours.enabled ? "text-purple-400" : "text-black/30 dark:text-white/20")} />
                                                        </div>
                                                        <div>
                                                            <span className="text-[11px] font-bold text-black/80 dark:text-white/80 block">Sessiz Saatler</span>
                                                            <span className="text-[9px] text-black/30 dark:text-white/20 font-bold uppercase tracking-tighter">İhbar Bildirim Aralığı</span>
                                                        </div>
                                                    </div>
                                                    <button 
                                                        ...
                                                        className={cn(
                                                            "w-11 h-6 rounded-full relative transition-all duration-300",
                                                            localSos.sosConfig.quietHours.enabled ? "bg-purple-500 shadow-[0_0_15px_rgba(168,85,247,0.3)]" : "bg-black/10 dark:bg-white/10"
                                                        )}
                                                    >
                                                        <div className={cn(
                                                            "absolute top-1 w-4 h-4 rounded-full bg-card transition-all shadow-moffi-card",
                                                        ...
                                                        <div className="flex items-center gap-3">
                                                            <button 
                                                                ...
                                                                className={cn(
                                                                    ...
                                                                    activeTimePicker === 'from' ? "bg-cyan-500/10 border-cyan-500/50" : "bg-black/5 dark:bg-white/5 border-card-border"
                                                                )}
                                                            >
                                                                <span className="text-[10px] font-black text-black/50 dark:text-white/40 mb-2 uppercase tracking-widest leading-none">Başlangıç</span>
                                                                <span className="text-white text-2xl font-black">{localSos.sosConfig.quietHours.from}</span>
                                                            </button>
                                                            <div className="w-2 h-0.5 bg-black/10 dark:bg-white/10 rounded-full" />
                                                            <button 
                                                                onClick={() => setActiveTimePicker(activeTimePicker === 'to' ? null : 'to')}
                                                                className={cn(
                                                                    "flex-1 border rounded-2xl p-4 flex flex-col items-center transition-all",
                                                                    activeTimePicker === 'to' ? "bg-purple-500/10 border-purple-500/50" : "bg-black/5 dark:bg-white/5 border-card-border"
                                                                )}
                                                            >
                                                                <span className="text-[10px] font-black text-black/50 dark:text-white/40 mb-2 uppercase tracking-widest leading-none">Bitiş</span>
                                                            ...
                                                            {activeTimePicker && (
                                                                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden bg-black/40 border border-card-border rounded-[2rem] p-8">
                                                                    <div className="flex justify-center items-center gap-8">
                                                                        ...
                                                                        />
                                                                        <div className="text-black/30 dark:text-white/20 font-black text-2xl self-end mb-11">:</div>
                                                                        <TimeWheel 
                                                                            label="Dakika"
                                                                    ...
                                                                    <button onClick={() => setActiveTimePicker(null)} className="w-full mt-6 py-4 bg-black/10 dark:bg-white/10 hover:bg-white/15 text-white font-bold rounded-2xl text-xs transition-colors">Tamam</button>
                                                                </motion.div>
                                                            )}
                                                        </AnimatePresence>
                                                    </motion.div>
                                                ...
                                                <div className="flex items-center justify-between group cursor-pointer" onClick={() => setLocalSos((prev: any) => ({...prev, sosConfig: {...prev.sosConfig, emergencyBypass: !prev.sosConfig.emergencyBypass}}))}>
                                                    <div className="flex items-center gap-4">
                                                        <div className="w-10 h-10 rounded-xl bg-black/5 dark:bg-white/5 flex items-center justify-center">
                                                            <ShieldCheck className={cn("w-5 h-5", localSos.sosConfig.emergencyBypass ? "text-cyan-400 font-bold" : "text-black/30 dark:text-white/20")} />
                                                        </div>
                                                        <div>
                                                            <span className="text-[11px] font-bold text-black/80 dark:text-white/80 block group-hover:text-cyan-400 transition-colors">Kritik İhbar Bypass</span>
                                                            <span className="text-[9px] text-black/30 dark:text-white/20 font-bold uppercase tracking-tighter italic leading-none block mt-1">Acil durumlarda sessiz modu del</span>
                                                        </div>
                                                    </div>
                            ...
                            {activeTab === 'preview' && (
                                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
                                    <div className="text-center">
                                        <h4 className="text-[10px] font-black text-black/40 dark:text-white/30 uppercase tracking-[0.4em] mb-4">Canlı Künye Önizlemesi</h4>
                                        <div className="relative mx-auto w-full max-w-[280px] aspect-[9/17.5] bg-white dark:bg-black rounded-[3rem] border-[6px] border-[#1C1C1E] overflow-hidden shadow-2xl">
                                            {/* SIMULATED MIN-BROWSER FOR PREVIEW */}
                                            <div className="w-full h-full bg-white dark:bg-black flex flex-col items-center p-4 pt-10 text-center">
                                                ...
                                                {isLost ? (
                                                    <div className="mt-4 w-full bg-red-500/20 border border-red-500/30 rounded-2xl p-3">
                                                        ...
                                                        <p className="text-[8px] text-black/60 dark:text-white/60 line-clamp-3">{localSos.sosConfig.emergencyMessage}</p>
                                                    </div>
                                                ) : (
                                                    <div className="mt-4 w-full bg-black/5 dark:bg-white/5 rounded-2xl p-3">
                                                        ...
                                                        <p className="text-[8px] text-black/50 dark:text-white/40 italic">"Sahibim yanımda..."</p>
                                                    </div>
                                                )}
                                                ...
                                                {isLost && (
                                                    <div className="mt-4 w-full space-y-2">
                                                        ...
                                                        <div className="w-full h-6 bg-black/10 dark:bg-white/10 rounded-lg" />
                                                    </div>
                                                )}
                                            </div>
                                            {/* Notch */}
                                            <div className="absolute top-0 left-1/2 -translate-x-1/2 w-20 h-6 bg-card dark:bg-[#1C1C1E] rounded-b-2xl" />
                                        </div>
                                        <p className="text-[10px] text-black/30 dark:text-white/20 mt-4 font-bold uppercase tracking-widest">Bu alan künyeyi tarayan birinin göreceği arayüzdür.</p>
                                    </div>
                                </motion.div>
                            )}
