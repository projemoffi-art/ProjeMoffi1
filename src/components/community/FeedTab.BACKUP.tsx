...
import { Plus, X, ShieldAlert, Swords } from 'lucide-react';
import { cn } from '@/lib/utils';
import { ImmersivePostCard } from './ImmersivePostCard';
import { ExploreGrid } from './ExploreGrid';
import { PodyumCard } from './PodyumCard';
import { DailyQuestsBar, DEFAULT_DAILY_QUESTS, type DailyQuest } from './DailyQuestsBar';
import { DuelCard, DuelStoryViewer, DuelCreateModal } from './DuelSystem';
...
export function FeedTab({
    ...
    const [viewingDuelId, setViewingDuelId] = useState<string | null>(null);
    const [isDuelCreateOpen, setIsDuelCreateOpen] = useState(false);

    // ── Quest state
    const [quests, setQuests] = useState<DailyQuest[]>(DEFAULT_DAILY_QUESTS);
    const [streak, setStreak] = useState(0);
    const [coinBalance, setCoinBalance] = useState(0);
    const completedToday = quests.filter(q => q.completed).length;

    // ── Load active duels for stories bar
    useEffect(() => {
        if (!user?.id) return;
        supabase
            .from('duels')
            ...
            .limit(5)
            .then(({ data }) => setActiveDuels(data || []));

        // Load user's coin balance and streak
        supabase
            .from('profiles')
            .select('coin_balance, streak')
            .eq('id', user.id)
            .single()
            .then(({ data }) => {
                if (data) {
    ...
    const handleCompleteQuest = async (questId: string) => {
        ...
        const earned = quest.coinReward;
        setCoinBalance(prev => prev + earned);
        // TODO: Gerçek coin güncelleme supabase.rpc ile
    };
    ...
    return (
        ...
        >
            ...
            <div className="w-full flex gap-4 px-4 py-4 overflow-x-auto no-scrollbar snap-start shrink-0">
                ...
                {/* Active Duels in stories bar */}
                {activeduels.map(duel => (
                    <DuelCard
                        key={duel.id}
                        duel={duel}
                        currentUserId={user?.id}
                        onClick={() => setViewingDuelId(duel.id)}
                    />
                ))}

                {/* Open new duel button (if has active pet) */}
                {activePet && (
                    <div
                        className="flex flex-col items-center gap-1.5 shrink-0 cursor-pointer group"
                        onClick={() => setIsDuelCreateOpen(true)}
                    >
                        <div className="relative w-16 h-16 rounded-full p-[2.5px] bg-gradient-to-tr from-purple-500/40 via-violet-600/40 to-fuchsia-500/40 border border-dashed border-purple-400/30 flex items-center justify-center group-hover:border-purple-400/60 transition-all">
                            <div className="w-full h-full rounded-full bg-purple-500/10 flex items-center justify-center group-hover:bg-purple-500/20 transition-all">
                                <Swords className="w-6 h-6 text-purple-400/70 group-hover:text-purple-300" />
                            </div>
                        </div>
                        <span className="text-[9px] text-purple-400/60 font-bold tracking-wide group-hover:text-purple-300 transition-colors">Düello</span>
                    </div>
                )}
            ...
            <PodyumCard
                currentUserId={user?.id}
                currentUserPetId={activePet?.id}
                currentUserPetPhoto={activePet?.image || activePet?.avatar}
                currentUserPetName={activePet?.name}
                            ...
                            <div className="flex items-center gap-2.5">
                                <div className="relative shrink-0">
                                    <div className="w-8 h-8 rounded-full border border-red-500/30 p-[1.5px] bg-red-500/10 flex items-center justify-center overflow-hidden">
                                        {activePet.avatar ? (
                                            <img src={activePet.avatar} alt={activePet.name} className="w-full h-full object-cover rounded-full" />
                                        ) : (
                                ...
                                <div className="flex flex-col text-left">
                                    <div className="flex items-center gap-1.5">
                                        <h3 className="text-[11px] font-black text-white uppercase tracking-wide leading-none">{activePet.name}</h3>
                                        <span className="text-[7.5px] font-black text-red-500 dark:text-red-400 bg-red-500/15 border border-red-500/25 px-1 py-[1.5px] rounded uppercase tracking-wider leading-none">KAYIP</span>
                                    </div>
                                    <p className="text-[8.5px] font-bold text-red-200/50 uppercase tracking-wider mt-1 leading-none">Arama Kurtarma Sinyali Aktif</p>
                                </div>
                            </div>
                            <div className="flex items-center gap-2 shrink-0">
                                <button
                                    onClick={() => { setSosActivePet(activePet); setIsSOSCommandCenterOpen(true); }}
            ...
            {isLoading ? (
                ...
                <ExploreGrid posts={posts} onPostClick={onPostClickFromGrid} isLoading={isLoading} />
            ) : (
            ...
            {/* ── DUEL VIEWER OVERLAY ──────────────────────────────────────── */}
            <AnimatePresence>
                {viewingDuelId && user?.id && (
                    <DuelStoryViewer
                        duelId={viewingDuelId}
                        currentUserId={user.id}
                        onClose={() => setViewingDuelId(null)}
                        onCoinEarned={handleCoinEarned}
                    />
                )}
            </AnimatePresence>
            ...
            {/* ── DUEL CREATE MODAL ─────────────────────────────────────────── */}
            <AnimatePresence>
                {isDuelCreateOpen && activePet && user?.id && (
                    <DuelCreateModal
                        currentUserId={user.id}
                        currentPetPhoto={activePet.image || activePet.avatar || ''}
                        currentPetName={activePet.name}
                        onClose={() => setIsDuelCreateOpen(false)}
                        onCreated={() => {
                            setIsDuelCreateOpen(false);
                            // Reload duels
                            supabase
                                .from('duels')
                                .select('*')
