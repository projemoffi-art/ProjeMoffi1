            ...
            <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8">
                ...
                <div className="flex bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-2xl p-1">
                    <button
                        ...
                        className={cn("px-6 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all", activeTab === 'vets' ? "bg-rose-500 text-white" : "text-black/50 dark:text-white/40 hover:text-white")}
                    >
                        Veteriner Onayları
                    </button>
                    <button
                        onClick={() => setActiveTab('sos')}
                        className={cn("px-6 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all", activeTab === 'sos' ? "bg-rose-500 text-white" : "text-black/50 dark:text-white/40 hover:text-white")}
                    >
                        Aktif SOS
                    </button>
                </div>
                ...
                <GlassCard className="p-8">
                    <div className="flex items-center justify-between mb-8">
                        ...
                        <div className="relative">
                            <Search className="w-4 h-4 absolute left-4 top-1/2 -translate-y-1/2 text-black/50 dark:text-white/40" />
                            <input type="text" placeholder="Doktor veya klinik ara..." className="bg-black/50 border border-black/10 dark:border-white/10 rounded-xl pl-12 pr-4 py-3 text-sm text-white focus:outline-none focus:border-rose-500 w-64" />
                        </div>
                    </div>

                    <div className="w-full text-left border-collapse">
                        <div className="grid grid-cols-4 text-[10px] font-black text-black/50 dark:text-white/40 uppercase tracking-widest pb-4 border-b border-black/10 dark:border-white/10">
                            <div>Doktor Adı</div>
                            <div>Klinik</div>
                        ...
                        {vets.map(vet => (
                            <div key={vet.id} className="grid grid-cols-4 items-center py-4 border-b border-black/5 dark:border-white/5 hover:bg-black/5 dark:bg-white/5 transition-colors">
                                <div className="font-bold text-white text-sm">{vet.name || 'İsimsiz'}</div>
                                <div className="text-black/60 dark:text-white/60 text-sm">{vet.businessName || '-'}</div>
                                <div className="text-black/60 dark:text-white/60 text-sm font-mono">{vet.created_at ? new Date(vet.created_at).toLocaleDateString() : '-'}</div>
                ...
                <GlassCard className="p-8">
                    ...
                    {sosPets.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-12 text-center border-dashed border-rose-500/30 bg-rose-500/5 rounded-2xl">
                            ...
                            <p className="text-black/50 dark:text-white/40 max-w-sm text-sm">Bölgedeki tüm sağlık sinyalleri normal düzeyde.</p>
                        </div>
                    ) : (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {sosPets.map(pet => (
                                <div key={pet.id} className="bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-2xl p-4 flex flex-col gap-3">
                                    ...
                                    <div className="text-black/60 dark:text-white/60 text-xs bg-black/30 p-3 rounded-xl line-clamp-2">
