            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
                {assets.map((asset, i) => (
                    <GlassCard key={asset.id} className="p-6 group cursor-pointer hover:border-fuchsia-500/30 transition-colors">
                        <div className="aspect-square rounded-[1.5rem] bg-black/5 dark:bg-white/5 mb-6 flex items-center justify-center border border-black/10 dark:border-white/10 group-hover:bg-black/10 dark:bg-white/10 transition-colors relative overflow-hidden">
                            <PlaySquare className="w-8 h-8 text-black/30 dark:text-white/20" />
                            {asset.is_prime && (
                        ...
                        <div className="flex items-center justify-between">
                            <p className="text-black/50 dark:text-white/40 text-xs uppercase tracking-widest">{asset.type}</p>
                            <p className="text-black/60 dark:text-white/60 text-xs font-mono">
